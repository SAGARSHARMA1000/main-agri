...
/* ---------------- COMPONENTS (UNCHANGED LOGIC, RESPONSIVE STYLES ADDED) ---------------- */

const StatCard = ({ label, value, subtext, icon: Icon, colorClass, borderClass }) => (
  <div className={`bg-white p-4 sm:p-6 rounded-2xl shadow-sm border ${borderClass}`}>
    <div className="flex justify-between items-start mb-4">
      <div>
...
export default function FarmerPaymentWallet() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      <header className="max-w-5xl mx-auto mb-8">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
          ...
          <div className="flex items-center gap-2 text-emerald-800 font-bold text-lg sm:text-xl">
            <Wallet className="fill-current" size={22} />
            <span>My Farm Wallet</span>
          </div>

          <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
            <button className="flex items-center justify-center gap-2 text-sm font-medium text-gray-600 bg-white border border-gray-200 px-4 py-2 rounded-lg hover:bg-gray-50 w-full sm:w-auto">
              <Download size={16} /> Statement
            </button>
            <button className="flex items-center justify-center gap-2 text-sm font-medium text-blue-600 bg-blue-50 px-4 py-2 rounded-lg hover:bg-blue-100 w-full sm:w-auto">
              <Landmark size={16} /> Bank Settings
            </button>
          </div>
        </div>
      ...
      {/* Main Content */}
      <main className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        ...
        <div className="lg:col-span-2 space-y-6 sm:space-y-8">
          ...
          {/* Escrow Holdings */}
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="bg-yellow-50 p-4 border-b border-yellow-100 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
              <h3 className="font-bold text-yellow-800 flex items-center gap-2 text-sm sm:text-base">
                <Shield size={18} /> Payments in Escrow
              </h3>
              <span className="text-xs bg-white text-yellow-700 font-bold px-2 py-1 rounded border border-yellow-200 w-fit">
                {escrowHoldings.length} Active
              </span>
            </div>

            {escrowHoldings.map(item => (
              <div key={item.id} className="p-4 sm:p-6 border-b border-gray-50 last:border-0 hover:bg-gray-50">
                
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-4">
                  <div>
                    <h4 className="font-bold text-gray-800 text-sm sm:text-base wrap-break-word">
                      {item.contract}
                    </h4>
                ...
                <div className="bg-gray-50 rounded-lg p-3 text-sm flex flex-col sm:flex-row gap-4 sm:gap-8 border border-gray-100">
                  <div>
        ...
        {/* Right Column – Withdraw (UNCHANGED UI STRUCTURE) */}
        <div className="lg:col-span-1">
          {/* Withdraw UI remains exactly same */}
        </div>
